...
$jscomp.asyncExecutePromiseGenerator=function(a){function b(g){return a.next(g)}function e(g){return a.throw(g)}return new Promise(function(g,d){function k(c){c.done?g(c.value):Promise.resolve(c.value).then(b,e).then(k,d)}k(a.next())})};$jscomp.asyncExecutePromiseGeneratorFunction=function(a){return $jscomp.asyncExecutePromiseGenerator(a())};$jscomp.asyncExecutePromiseGeneratorProgram=function(a){return $jscomp.asyncExecutePromiseGenerator(new $jscomp.generator.Generator_(new $jscomp.generator.Engine_(a)))};
$jscomp.initSymbol=function(){};$jscomp.polyfill("Symbol",function(a){if(a)return a;var b=function(k,c){this.$jscomp$symbol$id_=k;$jscomp.defineProperty(this,"description",{configurable:!0,writable:!0,value:c})};b.prototype.toString=function(){return this.$jscomp$symbol$id_};var e="jscomp_symbol_"+(1E9*Math.random()>>>0)+"_",g=0,d=function(k){if(this instanceof d)throw new TypeError("Symbol is not a constructor");return new b(e+(k||"")+"_"+g++,k)};return d},"es6","es3");
...
c.callWhenSettled_(f.resolve,f.reject)};d.prototype.settleSameAsThenable_=function(c,f){var h=this.createResolveAndReject_();try{c.call(f,h.resolve,h.reject)}catch(l){h.reject(l)}};d.prototype.then=function(c,f){function h(n,p){return"function"==typeof n?function(q){try{l(n(q))}catch(r){m(r)}}:p}var l,m,t=new d(function(n,p){l=n;m=p});this.callWhenSettled_(h(c,l),h(f,m));return t};d.prototype.catch=function(c){return this.then(void 0,c)};d.prototype.callWhenSettled_=function(c,f){function h(){switch(l.state_){case 1:c(l.result_);
break;case 2:f(l.result_);break;default:throw Error("Unexpected state: "+l.state_);}}var l=this;null==this.onSettledCallbacks_?k.asyncExecute(h):this.onSettledCallbacks_.push(h);this.isRejectionHandled_=!0};d.resolve=e;d.reject=function(c){return new d(function(f,h){h(c)})};d.race=function(c){return new d(function(f,h){for(var l=$jscomp.makeIterator(c),m=l.next();!m.done;m=l.next())e(m.value).callWhenSettled_(f,h)})};d.all=function(c){var f=$jscomp.makeIterator(c),h=f.next();return h.done?e([]):new d(function(l,
m){function t(q){return function(r){n[q]=r;p--;0==p&&l(n)}}var n=[],p=0;do n.push(void 0),p++,e(h.value).callWhenSettled_(t(n.length-1),m),h=f.next();while(!h.done)})};return d},"es6","es3");
var program=new function(){var a=function(){return $jscomp.asyncExecutePromiseGeneratorProgram(function(d){$("#box-subscrption").hide().siblings().hide();$("button").button();$("#btn-paypal-show").click(g);d.jumpToEnd()})},b=function(d,k){d=$("#btn-paypal-show").attr("plan-id");return k.subscription.create({plan_id:d,application_context:{shipping_preference:"NO_SHIPPING"}})},e=function(d,k){$("#btn-paypal-approve").attr("sub-data",JSON.stringify(d)).click()},g=function(){$("#box-subscrption").show().siblings().hide();
paypal.Buttons({style:{shape:"rect",color:"gold",layout:"vertical",label:"subscribe"},createSubscription:b,onApprove:e,onCancel:function(d){},onError:function(d){}}).render("#paypal-buttons")};this.main=function(){return a.apply(this,arguments)}};$(function(){program.main()});
